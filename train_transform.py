...
from net_transform import Mnet
import torch
...
# def my_loss1(epoch, score1, score2, score3, score4, s1_sig, s2_sig, s3_sig, s4_sig, label):
#     if epoch <= 60:
...
#         # label_g = F.interpolate(label, score4.shape[2:], mode='bilinear', align_corners=True)
#         sal_loss_g = F.binary_cross_entropy_with_logits(score4, label, reduction='mean')
#         return sal_loss1 + sal_loss2 + sal_loss3 + 0.5 * sml + 0.7*sal_loss_g
#     else:
#         sal_loss2 = F.binary_cross_entropy_with_logits(score2, label, reduction='mean')
#         sal_loss3 = F.binary_cross_entropy_with_logits(score3, label, reduction='mean')
...
#
#         return loss1 + loss2 + loss3 + 0.5 * sml + 0.7*loss4
    ...
    try:
        ...
        if epoch == 1:
            ...
            if best_avg_loss >= loss_all:
                ...
                torch.save(model.state_dict(), save_path + 'Best_AVG_epoch.pth')
    ...
    with torch.no_grad():
        ...
        if epoch==1:
            ...
            if mae<best_mae:
                ...
                torch.save(model.state_dict(), save_path+'Best_MAE_epoch.pth')
