    ...
    try:
        ...
        if epoch == 1:
            ...
            if best_avg_loss >= loss_all:
                ...
                torch.save(model.state_dict(), save_path + 'SPNet_noSPCM_Best_AVG_epoch.pth')
                print('best avg epoch:{}'.format(epoch))
        if epoch == 200:
            torch.save(model.state_dict(), save_path + 'SPNet_noSPCM.pth'.format(epoch))
    ...
    with torch.no_grad():
        ...
        if epoch==1:
            ...
            if mae<best_mae:
                ...
                torch.save(model.state_dict(), save_path+'SPNet_noSPCM_Best_MAE_epoch.pth')
                print('SPNet mae epoch:{}'.format(epoch))

if __name__ == '__main__':
    print("SPNet_noSPCM_Start train...")
