...
if __name__ == '__main__':
    for i in ['MAE']:   #,'AVG',
        test_datasets = ['NLPR','NJU2K']
        for dataset in test_datasets:
            time_s = time.time()
            model_path = os.path.join('./model/rgb_d/', 'SPNet_Best_' + str(i) + '_epoch_25p.pth') #Best_AVG_  ，  epoch_180   best_MAE_epoch
            model.load_state_dict(torch.load(model_path))
            sal_save_path = os.path.join('./output/', dataset + '-' + str(i) + '-SPNet_25/')
