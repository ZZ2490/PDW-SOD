    ...
    try:
        ...
        if epoch == 1:
            ...
            if best_avg_loss >= loss_all:
                ...
                torch.save(model.state_dict(), save_path + 'SPNet_Best_AVG_epoch_25p.pth')
                print('best avg epoch:{}'.format(epoch))
        if epoch == 200:
            torch.save(model.state_dict(), save_path + 'epoch_{}_SPNet_25p.pth'.format(epoch))
...
def val(test_loader,model,epoch,save_path):
    ...
    with torch.no_grad():
        ...
        print('SPNet_25p_Epoch: {} -MAE: {}  ...  bestmae: {} ...  best mae Epoch: {}'.format(epoch,mae,best_mae,best_mae_epoch))
        if epoch==1:
            best_mae = mae
        else:
            if mae<best_mae:
                ...
                torch.save(model.state_dict(), save_path+'SPNet_Best_MAE_epoch_25p.pth')
                print('SPNet mae epoch:{}'.format(epoch))

if __name__ == '__main__':
    print("SPNet_25p_Start train...")
