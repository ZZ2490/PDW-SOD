...
parser.add_argument('--test_path', type=str, default='../RGBT_dataset/test/VT1000/', help='test dataset path')
...
if __name__ == '__main__':
    for i in ['MAE',]:   #,'AVG',
        test_datasets = ['vt1000']
        for dataset in test_datasets:
            time_s = time.time()
            model_path = os.path.join('./model/', 'tha_gwm_pgfm_Best_' + str(i) + '_epoch_15p.pth') #Best_AVG_  ，  epoch_180   best_MAE_epoch
            model.load_state_dict(torch.load(model_path))
            sal_save_path = os.path.join('./output/', dataset + '-' + str(i) + '-821tha_gwm_pgfm_15p/')
