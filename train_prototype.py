...
from Encoder_Prototype_newbaseline import Mnet
    ...
    try:
        ...
        if epoch == 1:
            ...
            if best_avg_loss >= loss_all:
                ...
                torch.save(model.state_dict(), save_path + '5000tha_gwm_pgfm_Best_AVG_epoch_20p.pth')
                print('best avg epoch:{}'.format(epoch))
        if epoch == 200:
            torch.save(model.state_dict(), save_path + '5000tha_gwm_pgfm_epoch_{}_20p.pth'.format(epoch))
...
def val(test_loader,model,epoch,save_path):
    ...
    with torch.no_grad():
        ...
        print('tha_gwm_pgfm_Epoch: {} -MAE: {}  ...  bestmae: {} ...  best mae Epoch: {}'.format(epoch,mae,best_mae,best_mae_epoch))
        if epoch==1:
            best_mae = mae
        else:
            if mae<best_mae:
                ...
                torch.save(model.state_dict(), save_path+'5000tha_gwm_pgfm_Best_MAE_epoch_20p.pth')
                print('tha_gwm_pgfm_best mae epoch:{}'.format(epoch))

if __name__ == '__main__':
    print("20tha_gwm_pgfm_Start train...")
